...
export class UsersComponent implements OnInit {
  ...
  applyfilter(){
    ...
    if(!clearstatus){
      for(var j=0; j<this.sportlist.length; j++){
        ...
        this.postservice.getconnectusersfilter(1, this.sportlist[j].title, agestatus, age, genderstatus).subscribe(res=>{
          console.log(res);
          for(const i in res){
            this.users.push({
      ...
      for(var j=0; j<this.sportlist.length; j++){
        ...
        if(this.sportlist[j].status == true){
          ...
          this.postservice.getconnectusersfilter(0, this.sportlist[j].title, 0, 90, "ALL").subscribe(res=>{
            console.log(res);
            for(const i in res){
              this.users.push({
                username: res[i].UserName,
  ...
  getconnectusers(){
    this.postservice.getconnectusers().subscribe(res=>{
